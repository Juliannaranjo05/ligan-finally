...
/**
 * ModelCallHistoryList - Componente mejorado para el historial de llamadas (modelo)
 * Incluye agrupación por fecha, filtros y búsqueda
 */
const ModelCallHistoryList = ({ 
  callHistory = [], 
                      ...
                      <button
                        ...
                        className={`p-2 rounded-full transition-all duration-200 shadow-md ${
                          isCallActive || isReceivingCall
                            ? 'bg-gray-500/20 opacity-50 cursor-not-allowed shadow-none'
    ...
    <div className="flex flex-col flex-1 min-h-0 border-t border-[#ff007a]/10">
      ...
      <div className="flex-shrink-0 bg-[#2b2d31] pt-2 pb-2 z-10 px-3 sm:px-4">
        ...
        <div className="relative mb-2">
          ...
          <input
            type="text"
            placeholder={t("client.history.search") || t("clientInterface.searchHistory") || "Buscar en historial..."}
            value={searchQuery}
            ...
            aria-label={t("client.history.search") || t("clientInterface.searchHistory") || "Buscar en historial"}
          />
        </div>

        {/* Filtros - Mejorados */}
        <div className="flex items-center gap-1 bg-[#1f2125] rounded-lg p-1 border border-[#ff007a]/20 shadow-inner">
          <button
            onClick={() => setFilterType('all')}
            className={`px-3 py-1.5 text-xs rounded-md transition-all duration-200 font-medium ${
              filterType === 'all'
                ? 'bg-gradient-to-r from-[#ff007a] to-[#e6006e] text-white shadow-md shadow-[#ff007a]/30'
...
export default memo(ModelCallHistoryList);
