...
const ModelConversationList = ({
  ...
  return (
    <aside 
      ...
      role="complementary"
      aria-label={t('chat.title') || "Conversaciones"}
    >
      {/* Header sidebar móvil */}
      {isMobile && (
        <div className="flex justify-between items-center p-4 border-b border-[#ff007a]/20 flex-shrink-0">
          <h2 className="text-lg font-semibold text-white">{t("chat.title") || "Conversaciones"}</h2>
          <button
            onClick={onCloseSidebar}
            className="text-white/60 hover:text-white transition-colors p-2 hover:bg-[#3a3d44] rounded-lg"
      ...
      <div className="flex-1 overflow-hidden flex flex-col p-4">
        ...
        <div className="relative mb-4 flex-shrink-0">
          ...
          <input
            ...
            aria-label={t('chat.searchPlaceholder') || "Buscar conversaciones"}
          />
        </div>
            ...
            filteredConversations.map((conv, index) => {
              ...
              return (
                <div
                  ...
                  role="button"
                  tabIndex={0}
                  aria-label={`${displayName}, ${isOnline ? t('chat.online') : t('chat.offline')}${unreadCount > 0 ? `, ${unreadCount} ${t('chat.unreadMessages') || 'mensajes no leídos'}` : ''}`}
                  onKeyDown={(e) => {
