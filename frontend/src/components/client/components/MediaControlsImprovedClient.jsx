...
const MediaControlsImproved = ({ 
  ...
  useEffect(() => {
    ...
    const verifyCameraState = () => {
      ...
      // 🔥 RESPETAR LA DECISIÓN DEL USUARIO: Si la cámara está desactivada explícitamente, no reactivarla
      if (cameraEnabled === false) {
        return; // No verificar ni reactivar si el usuario la desactivó
      }
      ...
      if (localParticipant.videoTrackPublications) {
        for (const [trackSid, publication] of localParticipant.videoTrackPublications.entries()) {
          if (publication.source === Track.Source.Camera) {
            ...
            // 🔥 SOLO reactivar si hay publicación activa, track habilitado, NO está muted Y el usuario no la desactivó
            // Si está muted, significa que el usuario la desactivó explícitamente
            if (isEnabled && isSubscribed && !isMuted) {
    ...
    const handleTrackUnmuted = (track) => {
      ...
      if (track.kind === 'audio' && setMicEnabled) {
        ...
        // Solo actualizar si no está explícitamente desactivada por el usuario
        if (cameraEnabled !== false) {
          setCameraEnabled(true);
        }
      }
